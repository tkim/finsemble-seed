...
(function (definition) {
	...
	quoteFeedSunGard.loadDependencies=function(tryLater){
		if(typeof(muse)!="object" && !this.loadingDependencies){
			...
			if(!quoteFeedSunGard.museParams.group ||
		...
		if(this.loadingDependencies){
			if(typeof muse=="object"){
				...
				if(!(quoteFeedSunGard.museParams.server instanceof Array)) quoteFeedSunGard.museParams.server=[quoteFeedSunGard.museParams.server];
	...
	quoteFeedSunGard.connectToMuse=function (onsuccess, onfail) {
		...
		function getEntitlements(result){
			...
			var request={};
			request[reqID]={"Symbol":"MARKETS.UMS","Fields":[5507,5508,5509,5510]};
		...
		muse.config({
			...
			quoteFeedSunGard.connecting=false;
			if(onfail) onfail({ error: ((response && response.message) ? response.message : status) });
		});
	};
					...
					muse.monitorCharts(JSON.stringify(request), function (data) {
						if(!stx.isHistoricalMode()) {
							translateAndSendToChart(data, symbol, function(quotes) {
								...
								if(qts && qts.length) {
									qts=stx.quoteDriver.cleanup(stx,stx.chart,qts,CIQ.QuoteFeed.UPDATE,{symbol:symbol, chart:stx.chart});
	...
	quoteFeedSunGard.snapshotQuotes=function(symbols, cb){
		...
		function onConnected(data) {
			...
			var request={}, quoteData={}, reqMap={}, doneCnt=0;
				...
				for(var r in response){
					...
					if(data.NbMatch){
						for(var i=0;i<data.Values.length;i++){
							...
							var a;
							if(sym.indexOf("=")>-1){
								a=sym.split("=");a[a.length-1]="FOREX";
