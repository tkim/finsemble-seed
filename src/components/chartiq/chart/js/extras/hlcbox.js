...
(function (definition) {
	...
	CIQ.ChartEngine.prototype.drawHLCBox=function(params){
		var shaded=params.shaded, panel=params.panel, fillColor=params.fillColor, borderColor=params.borderColor, shadeColor=params.shadeColor, widthFactor=params.widthFactor;
		...
		for(var x=0;x<=quotes.length;x++){
			...
			if(pxTop!=pxBottom){
				...
				context.rect(xstart-highlightOffset, Math.min(pxTop,pxBottom)-highlightOffset, Math.max(1,xend-xstart+2*highlightOffset), Math.max(1,Math.abs(pxBottom-pxTop)+2*highlightOffset));
		...
		if(shadeColor && !CIQ.isTransparent(shadeColor)){
			...
			for(c=0;c<cache.length;c++){
				...
				for(c=0;c<cache.length;c++){
					point=cache[c]; pl=point.left; pr=point.right; pc=point.close; pt=point.top; pb=point.bottom;
					if(shaded){
