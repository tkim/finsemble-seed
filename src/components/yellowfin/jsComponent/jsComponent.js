...
function injectReport(uuid, elementId, opts) {
	...
	let titleHeight = parseInt(getComputedStyle(document.body).getPropertyValue('--titleBar-height').replace(/\D/g, ''));
	...
	options.height -= (parseInt(getComputedStyle(document.querySelector("div#buttons")).getPropertyValue('height').replace(/\D/g, ''))+1);
