...
var routerTransport_1 = require("./routerTransport");
...
exports.RouterClientConstructor = function (params) {
    ...
    function constructor(clientName, transportName) {
        ...
        function processManifest() {
            console.log("WINDOW LIFECYCLE:STARTUP:RouterClient:processManifest");
            //If manifest is a string, then there was an error getting the manifest because in a separate application
            asyncConnectToEventRouter(clientName, transportName, onReadyCallBack); /**** establish connection to router service ****/
        }
        processManifest();
    }
    ...
    function asyncConnectToEventRouter(clientName, transportName, onReadyCallBack) {
        ...
        function getClientTransport() {
            transport = routerTransport_1["default"].getTransport(routerParams, transportName, incomingMessageHandler, clientName, "RouterService")
                .then(transportReady)["catch"](errHandler);
        }
    ...
    function addQueryResponseTimeout(mapQueryResponseTimeOut, newQueryID, channel, timeout) {
        if (timeout > 0) {
            mapQueryResponseTimeOut[newQueryID] = setTimeout(function () {
                console.log("RouterClient: timeout waiting on query response on channel " + channel + " for queryID " + newQueryID +
     ...
     * @private
     */
    this.ready = function (cb) { return _this.onReady(cb); };
    /**
 ...
 */
    ...
    this.addListener = function (channel, eventHandler) {
        console.log("RouterClient.addListener", "CHANNEL", channel);
    ...
    this.transmit = function (toChannel, event, options) {
        ...
        if (options === void 0) { options = { suppressWarnings: false }; }
    ...
    this.addResponder = function (channel, queryEventHandler) {
        console.log("RouterClient.addResponder", "CHANNEL", channel);
    ...
    this.query = function (responderChannel, queryEvent, params, responseEventHandler) {
        if (responseEventHandler === void 0) { responseEventHandler = Function.prototype; }
        ...
        function promiseResolver(resolve) {
            ...
            var modifiedHandler = function (err, response) {
