...
const sslOptions = {
	...
	cert: fs.readFileSync(
		path.join(__dirname, "certs/localhost.chartiq.com.crt")
	),
...
module.exports.updateServer = (app, cb) => {
	...
	app.post("/login", upload.fields([]), function(req, res, next) {
		...
		if (typeof req.body === "string") {
			...
			return res
				.status(401)
				.send("No credentials received. Invalid body format.");
		}
		try {
	...
	const {
		restFinsembleActions,
		subscribe
	} = require("./rest-router/restRouter");
	const { spawn, publish } = restFinsembleActions;

	app.get("/finsemble-rest/spawn/:component/:params?", (request, response) => {
	...
	app.get("/finsemble-rest/publish/:topic/:data", (request, response) => {
		const { topic, data } = request.params;
		console.log(publish(topic, data));
		publish(topic, data);
		response.send(`Published ${data} to ${topic}`);
	});
	app.get("/finsemble-rest/subscribe/:topic", async (request, response) => {
